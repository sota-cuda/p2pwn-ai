...
func printHelp() {
	...
	fmt.Printf("[-d, --debug] Enable debug logging (writes debug.txt to output folder)\n")
...
func main() {
	...
	if rebrandMode {
		if output == "" {
			printErrorAndExit("Output folder required for rebrand mode")
		}
	...
	// Run scanner
	core.InitDebug(output, debugMode)
